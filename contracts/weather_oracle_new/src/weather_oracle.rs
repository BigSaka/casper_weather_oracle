...
impl WeatherOracle {
    ...
    pub fn submit_reading(
        ...
        let agent = self.agent.get_or_revert_with(Error::NotAuthorizedAgent);
        if caller != agent {
    ...
    /// Admin/view: the registered agent address. Reverts if somehow
    /// called before `init` set it (should not happen in practice).
    pub fn get_agent(&self) -> Address {
        self.agent.get_or_revert_with(Error::NotAuthorizedAgent)
    }
}